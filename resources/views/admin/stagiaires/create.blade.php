...
<div class="space-y-6 max-w-3xl">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-100 flex items-center gap-3">
                <i class="fa-solid fa-user-plus text-indigo-400"></i> Ajouter un Stagiaire
            </h1>
            <p class="text-slate-400 text-sm mt-1">Enregistrez un nouveau stagiaire et créez automatiquement son accès système</p>
        </div>
    ...
    <div class="glass-panel p-6 sm:p-8 rounded-2xl border border-slate-800">
        <form action="{{ route('admin.stagiaires.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Nom *</label>
                    <input type="text" name="nom" value="{{ old('nom') }}" required placeholder="Ex: MBARGA" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Prénom *</label>
                    <input type="text" name="prenom" value="{{ old('prenom') }}" required placeholder="Ex: Jean Paul" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="stagiaire@domaine.cm" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Téléphone</label>
                    <input type="text" name="telephone" value="{{ old('telephone') }}" placeholder="Ex: 699000000" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 font-mono">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="naissance" value="{{ old('naissance') }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Lieu de naissance</label>
                    <input type="text" name="lieu_naissance" value="{{ old('lieu_naissance') }}" placeholder="Ex: Yaoundé" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Lieu / Ville de résidence</label>
                    <input type="text" name="lieu" value="{{ old('lieu') }}" placeholder="Ex: Douala" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-200 mb-2">Photo de profil (Optionnel)</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-200 mb-2">Mot de passe de connexion initial *</label>
