...
<div class="space-y-6 max-w-2xl">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-100 flex items-center gap-3">
                <i class="fa-solid fa-user-pen text-indigo-400"></i> Modifier l'Encadrant
            </h1>
            <p class="text-slate-400 text-sm mt-1">Mise à jour des informations de {{ $encadrant->nom }}</p>
        </div>
    ...
    <div class="glass-panel p-6 sm:p-8 rounded-2xl border border-slate-800">
        <form action="{{ route('admin.encadrants.update', $encadrant) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-semibold text-slate-200 mb-2">Nom et Prénom *</label>
                <input type="text" name="nom" value="{{ old('nom', $encadrant->nom) }}" required class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-200 mb-2">Adresse Email *</label>
                <input type="email" name="email" value="{{ old('email', $encadrant->email) }}" required class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500">
            </div>
